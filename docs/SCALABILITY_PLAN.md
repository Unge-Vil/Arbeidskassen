...
- **2.5** — Suspense-grense i authenticated layout (nice-to-have, krever omstrukturering av `AuthenticatedShell`, kan tas i Fase 4)
...
- [ ] Hver modul har egen `layout.tsx` med modulspesifikk data-fetching — organisasjon/bookdet/teamarea har det; today/dashboard mangler separate layouts
...
- [x] Hver modul har `loading.tsx` og `error.tsx` — 8 moduler verifisert
- [ ] TTFB for dashboardsiden < 200ms i produksjon — krever produksjonsmåling

### Fase 5 — Infrastruktur

- [ ] Rate limiting fungerer faktisk — ⚠️ **Krever Upstash Redis-oppsett** (se 5.1 under)
- [ ] Bundle-analyse gjennomført — kjør `ANALYZE=true pnpm build` fra `apps/arbeidskassen/`
