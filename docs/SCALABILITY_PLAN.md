...
| **Cleanup: slettet døde flate i18n-filer** | ✅ **Utført nå** | Se nedenfor |
| **Cleanup: fjernet vestigielle locale-parametere** | ✅ **Utført nå** | Se nedenfor |
| **DRY middleware `protectedPrefixes`** | ✅ **Utført 14. apr** | Ekstrahert felles `authPolicy`-objekt |
| **Clean build: fjernet dashboard static gen warning** | ✅ **Utført 14. apr** | `force-dynamic` på `/dashboard`, ryddet catch-blokk |
| **Dashboard catch-blokk forenklet** | ✅ **Utført 14. apr** | Fjernet unødig error-logging i `dashboard.ts` |
...
### Cleanup: slettet døde flate i18n-filer

De gamle flate meldingsfilene `messages/no.json` og `messages/en.json` (475 linjer hver) var foreldet — `i18n/request.ts` laster kun fra per-namespace-kataloger (`messages/no/`, `messages/en/`). Filene ble slettet.

### Cleanup: fjernet vestigielle locale-parametere

Etter at `[locale]` ble fjernet fra URL-stier (3.1), hadde mange hjelpefunksjoner i `admin-links.ts` fortsatt `_locale`-parametere som aldri ble brukt. Fjernet fra:

- `resolveAdminAppHrefs()` — 10+ kall-steder oppdatert
- `resolveInternalAdminHref()` — tester oppdatert
- `buildArbeidskassenHref()` — tester oppdatert
- `buildLocalizedAppHref()` — tester oppdatert
- `normalizeReturnTo()` og `appendReturnTo()` — intern opprydding
- `DashboardGrid.locale` prop — fjernet fra grensesnitt, grid-kode og begge kall-steder
- `AppIconWidget.currentLocale` og `QuickActionsWidget.currentLocale` — fjernet fra props

Resultat: `next-intl`-import fjernet fra `backoffice/page.tsx` og `dashboard/page.tsx` klient-bundler. Bundle-størrelse ned:
- Backoffice: 240 → **227 kB** (−13 kB)
- Dashboard: 295 → **281 kB** (−14 kB)
- Bookdet sub-sider: 294 → **293 kB** (−1 kB)

### DRY middleware `protectedPrefixes` — 14. april 2026

`apps/arbeidskassen/middleware.ts` hadde `protectedPrefixes`-arrayet duplisert mellom API-branchen og page-branchen. Ekstrahert til felles `authPolicy`-objekt som brukes begge steder. Reduserte ~20 dupliserte linjer til én felles referanse.

### Clean build: fjernet dashboard static gen warning — 14. april 2026

Under `pnpm build` logget Next.js en noisy `Dashboard fetch error: Dynamic server usage` fordi `/dashboard`-siden ble forsøkt statisk generert, men bruker `cookies()`. Fikset med:
- `export const dynamic = "force-dynamic"` i `dashboard/page.tsx` — forteller Next.js å hoppe over static gen-forsøket
- Forenklet catch-blokk i `packages/supabase/src/dashboard.ts` — fjernet unødig error-logging siden feilen er harmløs

Resultat: `pnpm build` er nå helt ren uten advarsler. Static generation gikk fra 29 → 28 sider (dashboard hopper over).
...
- [x] `/login` og landingssiden gjør null server-side Supabase-kall — verifisert: login er `"use client"`, bruker kun `createBrowserClient`. Middleware kaller `getUser()` for redirect-sjekk (nødvendig).
...
- [ ] `next build` viser `○ (Static)` for minst noen sider — **Ikke oppnåelig uten PPR**: alle sider bruker `cookies()` via root layout (i18n + tema). Krever PPR (5.3) for å bli statisk.
