...
export function TeamAreaShell({
  ...
  const navigationItems = [
    {
      href: `/?view=feed&group=${currentGroup}`,
      view: "feed",
      label: t("nav.feed"),
      shortLabel: "N",
    },
    {
      href: `/?view=saved&group=${currentGroup}`,
      view: "saved",
      label: t("nav.saved"),
      shortLabel: "S",
    },
    {
      href: `/?view=announcements&group=${currentGroup}`,
      view: "announcements",
  ...
  const groups = [
    {
      href: `/?view=${currentView}&group=all-hands`,
      slug: "all-hands",
      label: t("groups.allHands"),
      shortLabel: "A",
    },
    {
      href: `/?view=${currentView}&group=design-team`,
      slug: "design-team",
      label: t("groups.designTeam"),
      shortLabel: "D",
    },
    {
      href: `/?view=${currentView}&group=operations`,
      slug: "operations",
  ...
  const isNavActive = (view: string) => pathname === `/${locale}` && currentView === view;
  const isGroupActive = (slug: string) => pathname === `/${locale}` && currentGroup === slug;

  return (
    <div className="flex h-screen w-full select-none flex-col overflow-hidden bg-[#07090d] font-sans text-[var(--ak-text-main)] transition-colors duration-300">
      <Navbar
        workspaceName="TeamArea"
        workspaceInitial="T"
        locale={locale}
        orgName={orgName}
        ...
        <div className="flex h-full w-full flex-col lg:flex-row">
          ...
          <main className="min-w-0 flex-1 overflow-y-auto bg-[#05070b]">
            <div className="mx-auto w-full max-w-5xl px-4 py-5 lg:px-8 lg:py-8">
              {isPreviewMode ? (
                <div className="mb-4 rounded-xl border border-[var(--ak-border-soft)] bg-[var(--ak-bg-panel)]/90 px-4 py-3 text-sm text-[var(--ak-text-muted)]">
                  TeamArea kjører i forhåndsvisning uten Supabase-tilkobling lokalt akkurat nå.
                </div>
              ) : null}
              {children}
            </div>
