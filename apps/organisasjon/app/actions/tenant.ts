...
export async function updateTenantSettingsAction(formData: FormData) {
  ...
  const redirectBase = `/${currentLocale}/virksomhet`;
  ...
  revalidatePath(`/${currentLocale}/virksomhet`);
  revalidatePath(`/${targetLocale}/virksomhet`);
  revalidatePath(`/${currentLocale}`);
  revalidatePath(`/${targetLocale}`);
  revalidatePath("/", "layout");

  redirect(`/${targetLocale}/virksomhet?saved=1`);
}
